...
* **Smart Error Handling:** If the limit is exceeded, it preserves the original subscription name and data usage headers, but replaces the configuration list with a clear placeholder node (e.g., `⚠️_DEVICE_LIMIT_REACHED`).
...
3. If it's a new device and the limit **is reached**, it checks for inactive ("expired") devices based on your TTL settings to replace them. If none are found, it blocks the request by returning a visual warning configuration.
